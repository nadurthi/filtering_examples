% function Kirb_sims_12thJuly2012_allfilters_call2()
clear all
clc 
close all
...
pf.no_bins = 200;               % when computing the histogram

pf.resample = true;             % resampling after measurement update
pf.neff = 0.7;                  % treshold for the number of effective samples
pf.plotHists =false;
...
%% Create the NNN data sets in a cell to sent to parfor lloop
    ...
    [t,x_mc1]=ode45_discc(@(x)KIRB_UM_eg_dyn_disc(x,T),time.t0,time.dt,125,model.x0tr,1e-200);
    [t,x_mc2]=ode45_discc(@(x)KIRB_CT_eg_dyn_disc(x,T),t(end),time.dt,t(end)+90,[x_mc1(end,:)';1*pi/180],1e-200);
    [t,x_mc3]=ode45_discc(@(x)KIRB_UM_eg_dyn_disc(x,T),t(end),time.dt,t(end)+125,x_mc2(end,1:end-1)',1e-200);
    [t,x_mc4]=ode45_discc(@(x)KIRB_CT_eg_dyn_disc(x,T),t(end),time.dt,t(end)+30,[x_mc3(end,:)';-3*pi/180],1e-200);
    [t,x_mc5]=ode45_discc(@(x)KIRB_UM_eg_dyn_disc(x,T),t(end),time.dt,t(end)+125,x_mc4(end,1:end-1)',1e-200);
...
%         figure(1)
% plot(x_mc(:,1),x_mc(:,2),ym(:,1).*cos(ym(:,2)),ym(:,1).*sin(ym(:,2)),'k*')
% keyboard
    filter.ymeas=ym;
    filter.truth=x_mc;
 at = now;
...
aa(find(aa==' '))='_';
aa(find(aa==':'))='-';
    model.name=strcat('KIRB2_',aa,'_NNN_',num2str(i));
...
t=time.tspan;
figure
plot(t,est_fin_mupf(:,1),'-.',t,est_fin_mopf(:,1),':',t,est_fin_ckf(:,1),'-^',t,est_fin_ukf(:,1),'-+',t,est_fin_cut4(:,1),'-*',t,est_fin_cut6(:,1),'-d',t,est_fin_cut8(:,1),'-o',t,est_fin_gh(:,1),'-s','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
legend('PF-mu','PF-mo','ckf','ukf','cut4','cut6','cut8','GH6','Location','NorthWest')
xlabel('t (s)')
ylabel('RMSE_{pos} (m)')
% plot_prop_paper
% saveas(gcf,strcat('Kirb_pos1','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,est_fin_mupf(:,1),'-.',t,est_fin_mopf(:,1),':',t,est_fin_ckf(:,1),'-^',t,est_fin_ukf(:,1),'-+',t,est_fin_cut4(:,1),'-*','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','ckf','ukf','cut4','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{pos} (m)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_pos2','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,est_fin_mupf(:,1),'-.',t,est_fin_mopf(:,1),':',t,est_fin_cut4(:,1),'-*',t,est_fin_cut6(:,1),'-d',t,est_fin_cut8(:,1),'-o','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','cut4','cut6','cut8','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{pos} (m)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_pos3','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,est_fin_mupf(:,1),'-.',t,est_fin_mopf(:,1),':',t,est_fin_cut8(:,1),'-o',t,est_fin_gh(:,1),'-s','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','cut8','GH6','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{pos} (m)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_pos4','.eps'),'psc2') 
% % close
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure
plot(t,est_fin_mupf(:,2),'-.',t,est_fin_mopf(:,2),':',t,est_fin_ckf(:,2),'-^',t,est_fin_ukf(:,2),'-+',t,est_fin_cut4(:,2),'-*',t,est_fin_cut6(:,2),'-d',t,est_fin_cut8(:,2),'-o',t,est_fin_gh(:,2),'-s','LineWidth',2,'MarkerEdgeColor','k','MarkerSize',12)
legend('PF-mu','PF-mo','ckf','ukf','cut4','cut6','cut8','GH6','Location','NorthWest')
xlabel('t (s)')
ylabel('RMSE_{vel} (m/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_vel1','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,est_fin_mupf(:,2),'-.',t,est_fin_mopf(:,2),':',t,est_fin_ckf(:,2),'-^',t,est_fin_ukf(:,2),'-+',t,est_fin_cut4(:,2),'-*','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','ckf','ukf','cut4','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{vel} (m/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_vel2','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,est_fin_mupf(:,2),'-.',t,est_fin_mopf(:,2),':',t,est_fin_cut4(:,2),'-*',t,est_fin_cut6(:,2),'-d',t,est_fin_cut8(:,2),'-o','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','cut4','cut6','cut8','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{vel} (m/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_vel3','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,est_fin_mupf(:,2),'-.',t,est_fin_mopf(:,2),':',t,est_fin_cut8(:,2),'-o',t,est_fin_gh(:,2),'-s','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','cut8','GH6','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{vel} (m/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_vel4','.eps'),'psc2') 
% % close
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure
% plot(t,(180/pi)*est_fin_mupf(:,3),'-.',t,(180/pi)*est_fin_mopf(:,3),':',t,(180/pi)*est_fin_ckf(:,3),'-^',t,(180/pi)*est_fin_ukf(:,3),'-+',t,(180/pi)*est_fin_cut4(:,3),'-*',t,(180/pi)*est_fin_cut6(:,3),'-d',t,(180/pi)*est_fin_cut8(:,3),'-o',t,(180/pi)*est_fin_gh(:,3),'-s','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','ckf','ukf','cut4','cut6','cut8','GH6','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{\Omega} (deg/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_om1','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,(180/pi)*est_fin_mupf(:,3),'-.',t,(180/pi)*est_fin_mopf(:,3),':',t,(180/pi)*est_fin_ckf(:,3),'-^',t,(180/pi)*est_fin_ukf(:,3),'-+',t,(180/pi)*est_fin_cut4(:,3),'-*','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','ckf','ukf','cut4','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{\Omega} (deg/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_om2','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,(180/pi)*est_fin_mupf(:,3),'-.',t,(180/pi)*est_fin_mopf(:,3),':',t,(180/pi)*est_fin_cut4(:,3),'-*',t,(180/pi)*est_fin_cut6(:,3),'-d',t,(180/pi)*est_fin_cut8(:,3),'-o','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','cut4','cut6','cut8','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{\Omega} (deg/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_om3','.eps'),'psc2') 
% % close
% 
% figure
% plot(t,(180/pi)*est_fin_mupf(:,3),'-.',t,(180/pi)*est_fin_mopf(:,3),':',t,(180/pi)*est_fin_cut8(:,3),'-o',t,(180/pi)*est_fin_gh(:,3),'-s','LineWidth',2.0,'MarkerEdgeColor','k','MarkerSize',12)
% legend('PF-mu','PF-mo','cut8','GH6','Location','NorthWest')
% xlabel('t (s)')
% ylabel('RMSE_{\Omega} (deg/s)')
% plot_prop_paper
% % saveas(gcf,strcat('Kirb_om4','.eps'),'psc2') 
% % close
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure
plot(avg_traj_mupf(:,1),avg_traj_mupf(:,2),'-.',avg_traj_mopf(:,1),avg_traj_mopf(:,2),':',avg_traj_ckf(:,1),avg_traj_ckf(:,2),avg_traj_ukf(:,1),avg_traj_ukf(:,2),avg_traj_cut4(:,1),avg_traj_cut4(:,2),avg_traj_cut6(:,1),avg_traj_cut6(:,2),avg_traj_cut8(:,1),avg_traj_cut8(:,2,end),avg_traj_gh(:,1),avg_traj_gh(:,2),avg_traj_mc(:,1),avg_traj_mc(:,2),'--',YNNN_mc(:,1).*cos(YNNN_mc(:,2)),YNNN_mc(:,1).*sin(YNNN_mc(:,2)),'k*','LineWidth',2)
hold on
plot(0,0,'s','MarkerEdgeColor','k',...
                'MarkerFaceColor','g',...
                'MarkerSize',10)
legend('PF-mu','PF-mo','ckf','ukf','cut4','cut6','cut8','GH','True Path','Measurements','Sensor')
xlabel('x (m)')
ylabel('y (m)')
% 
% hold off
...
% 
% figure
% plot(avg_traj_mc(:,1),avg_traj_mc(:,2),'--','LineWidth',3)
% hold on
% plot(0,0,'s','MarkerEdgeColor','k',...
%                 'MarkerFaceColor','g',...
%                 'MarkerSize',10)
%  plot(avg_traj_mc(1,1),avg_traj_mc(1,2),'d','MarkerEdgeColor','k',...
%                 'MarkerFaceColor','r',...
%                 'MarkerSize',10)
% legend('True Path','Sensor')
% xlabel('x (m)')
% ylabel('y (m)')
% axis([-1.5,3,-2,1.5]*10^4)
% hold off
% plot_prop_paper
% 
...
% bb=(1)*sqrt([sum(est_fin_mupf(:,3).^2),sum(est_fin_mopf(:,3).^2),sum(est_fin_ckf(:,3).^2),sum(est_fin_ukf(:,3).^2),sum(est_fin_cut4(:,3).^2),sum(est_fin_cut6(:,3).^2),sum(est_fin_cut8(:,3).^2),sum(est_fin_gh(:,3).^2),sum(est_fin_ekf(:,3).^2)])
% fprintf(fid, '% 12.8f' , bb);
% fprintf(fid, '\n')
% fclose(fid);
...
a1=zeros(3,8);
a2=zeros(3,8);
a1(1,:)=[sqrt(sum(sqrt(sum(est_fin_mupf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_mopf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_ckf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_ukf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut4(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut6(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut8(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_gh(:,pp).^2/model.fn,2)).^2)/time.nSteps)] 
a2(1,:)=[max(max(est_fin_mupf(:,pp))),max(max(est_fin_mopf(:,pp))),max(max(est_fin_ckf(:,pp))),max(max(est_fin_ukf(:,pp))),max(max(est_fin_cut4(:,pp))),max(max(est_fin_cut6(:,pp))),max(max(est_fin_cut8(:,pp))),max(max(est_fin_gh(:,pp)))]

pp=2;
a1(2,:)=[sqrt(sum(sqrt(sum(est_fin_mupf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_mopf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_ckf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_ukf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut4(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut6(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut8(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_gh(:,pp).^2/model.fn,2)).^2)/time.nSteps)] 
a2(2,:)=[max(max(est_fin_mupf(:,pp))),max(max(est_fin_mopf(:,pp))),max(max(est_fin_ckf(:,pp))),max(max(est_fin_ukf(:,pp))),max(max(est_fin_cut4(:,pp))),max(max(est_fin_cut6(:,pp))),max(max(est_fin_cut8(:,pp))),max(max(est_fin_gh(:,pp)))]

pp=3;
a1(3,:)=[sqrt(sum(sqrt(sum(est_fin_mupf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_mopf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_ckf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_ukf(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut4(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut6(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_cut8(:,pp).^2/model.fn,2)).^2)/time.nSteps),sqrt(sum(sqrt(sum(est_fin_gh(:,pp).^2/model.fn,2)).^2)/time.nSteps)] 
a2(3,:)=[max(max(est_fin_mupf(:,pp))),max(max(est_fin_mopf(:,pp))),max(max(est_fin_ckf(:,pp))),max(max(est_fin_ukf(:,pp))),max(max(est_fin_cut4(:,pp))),max(max(est_fin_cut6(:,pp))),max(max(est_fin_cut8(:,pp))),max(max(est_fin_gh(:,pp)))]

s=datestr(now);
s(find(s==' '))='_';
s(find(s==':'))='-';
save(strcat(s,'_Kirb_eg'),'time','model','pf','filter','xNNN_mc','YNNN_mc','xNNN_mopf','xNNN_mupf','xNNN_ukf','xNNN_ckf','xNNN_cut4','xNNN_cut6','xNNN_cut8','xNNN_gh','xNNN_ekf','PF_complete_data','PNNN_ukf','PNNN_ckf','PNNN_cut4','PNNN_cut6','PNNN_cut8','PNNN_gh','PNNN_ekf','PNNN_pf','a1','a2','-v7.3')
% end