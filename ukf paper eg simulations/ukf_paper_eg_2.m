...
%     Meas_disc_Jac=@UKF_eg_meas_disc_Jac;
    ...
    % MC truth conditions
    x0tr=[6500.4,349.14,-1.8093,-6.7967,0.6932]';
    P0tr=diag([1e-6,1e-6,1e-6,1e-6,0]);
%  P0tr=diag([200,10,200,10,1e-2]);
    n=length(x0tr);
    
Qf=diag([0,0,2.4064*1e-5,2.4064*1e-5,1e-5]);
...
nR=length(R);
    ...
    x_0f=[6500.4,349.14,-1.8093,-6.7967,0]';
    P_0f=diag([1e-4,1e-4,2e-5,2e-5,1]);
    
%      ukf_eg_para_set = struct('freq', {}, 'R', {},'x0tr', {}, 'P0tr', {},'Qf', {}, 'Qt', {},'x_0f', {}, 'P_0f', {});
 ukf_eg_para_set = struct('freq', f, 'R', R,'x0tr', x0tr, 'P0tr', P0tr,'Qf', Qf, 'Qt', Qt,'x_0f', x_0f, 'P_0f', P_0f);
% ukf_eg_para_set.freq=f;
% ukf_eg_para_set.R=R;
% ukf_eg_para_set.x0tr=x0tr;
% ukf_eg_para_set.P0tr=P0tr;
% ukf_eg_para_set.Qf=Qf;
% ukf_eg_para_set.Qt=Qt;
% ukf_eg_para_set.x_0f=x_0f;
% ukf_eg_para_set.P_0f=P_0f;
...
NNN=1;
% initialization
x_mc=zeros(length(t0:dt:tF),n,NNN);
x100_ukf=zeros(length(t0:dt:tF),n,NNN);
x100_ckf=zeros(length(t0:dt:tF),n,NNN);
x100_cut4=zeros(length(t0:dt:tF),n,NNN);
x100_cut6=zeros(length(t0:dt:tF),n,NNN);
x100_cut8=zeros(length(t0:dt:tF),n,NNN);
x100_gh=zeros(length(t0:dt:tF),n,NNN);


P100_ukf=zeros(length(t0:dt:tF),n^2,NNN);
P100_ckf=zeros(length(t0:dt:tF),n^2,NNN);
P100_cut4=zeros(length(t0:dt:tF),n^2,NNN);
P100_cut6=zeros(length(t0:dt:tF),n^2,NNN);
P100_cut8=zeros(length(t0:dt:tF),n^2,NNN);
P100_gh=zeros(length(t0:dt:tF),n^2,NNN);
           est_fin_ukf=0;
           est_fin_ckf=0;
           est_fin_cut4=0;
           est_fin_cut6=0;
...
for j=1:1:NNN
    ...
    [t,X]=ode45_discc(Dyn_disc,t0,dt,tF,x0trr',Qt);
    %generating the measurement
    ym=zeros(length(t),nR);
...
% plot(X(:,1),X(:,2),ym(:,1).*cos(ym(:,2))+6374,ym(:,1).*sin(ym(:,2)),'k*')
% legend('MC','sensor')
...
%     Cov_ekf=reshape(Pu,1,n^2);
   ...
   %Filtering
    for tt=t0:dt:tF-dt
        disp(['freq = ',num2str(f),' Time step no. ',num2str(tt),' of MC Iteration no. ',num2str(j)]);
     ...
     % [mu_cut4,P_cut4]=QUADpts_disc_disc_modified(Dyn_disc,Meas_disc,mu_cut4,P_cut4,zm,Qf,R,'cut4',0);
      ...
      %[mu_gh,P_gh]=QUADpts_disc_disc_modified(Dyn_disc,Meas_disc,mu_gh,P_gh,zm,Qf,R,'gh',6); 
